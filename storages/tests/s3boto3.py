...
class S3BotoTestCase(TestCase):
    @mock.patch('storages.backends.s3boto3.Session')
    def setUp(self, S3Connection):
        self.storage = s3boto3.S3BotoStorage()
...
class S3BotoStorageE2ETests(TestCase):
    """
    End to end tests to test the boto3 integration.
    """

    def setUp(self):
        self.bucket_name = 's3boto3test-s3boto3test-s3boto3test'
        self.storage = s3boto3.S3BotoStorage(bucket=self.bucket_name)

    def tearDown(self):
        if hasattr(self.storage, '_bucket'):
            self.storage.bucket.delete()

    def test_bucket_does_not_exist(self):
        with self.assertRaises(ImproperlyConfigured):
            self.storage.bucket

    @mock.patch('storages.backends.s3boto3.AUTO_CREATE_BUCKET', True)
    def test_auto_create_bucket(self):
        self.assertTrue(self.storage.bucket)


class S3BotoStorageTests(S3BotoTestCase):
    def test_bucket_does_not_exist_exception(self):
        self.storage._bucket_exists = mock.Mock(return_value=False)
        with self.assertRaises(ImproperlyConfigured):
            self.storage._get_or_create_bucket(name='test')
    ...
    def test_auto_create_bucket(self):
        ...
        bucket_name = 'test'
        bucket = self.storage._get_or_create_bucket(name=bucket_name)

        bucketCollection = self.storage.BucketCollection.return_value
        bucketCollection.create.assert_called_with(bucket=bucket_name)
        bucket.put_acl.assert_called_with(acl=self.storage.bucket_acl)
    ...
    #    """
    #    if not s3boto3.IS_GZIPPED:  # Gzip not available.
    #        return
...
#        mode = 'w'
#        f = s3boto3.S3BotoStorageFile(name, mode, self.storage)
